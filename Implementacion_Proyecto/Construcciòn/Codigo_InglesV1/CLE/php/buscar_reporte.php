...
if ($Tipo == 'tecnico') {
	$sql = mysqli_query($conn, "SELECT * FROM reportes WHERE  fecha>='$ValorDe' AND fecha<='$ValorA' AND tecnico='$Usuario'  AND (atendido = 1 OR atendido != NULL) ");
}else{
	...
	$sql = mysqli_query($conn, "SELECT * FROM reportes WHERE  fecha>='$ValorDe' AND fecha<='$ValorA' AND id_cliente='$id_cliente'  AND atendido = 1");
}
echo $mensaje;
?>
