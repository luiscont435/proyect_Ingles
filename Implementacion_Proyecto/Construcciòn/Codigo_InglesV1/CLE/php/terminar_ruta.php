...
if(mysqli_query($conn, "UPDATE rutas SET estatus = 1  WHERE id_ruta = $id_ruta")){
	...
	echo '<script type="text/javascript">M.toast({html:"La ruta se actualizado correctamente.", classes: "rounded"})</script>';
}else{
	echo '<script type="text/javascript">M.toast({html:"Ocurrio un error y no se actualizo.", classes: "rounded"})</script>';
}
