...
$mensaje = "";
	...
	$sql = "UPDATE reportes SET falla = '$Falla', solucion = '$Solucion', tecnico = '$Tecnico', atendido = '$Atendido', fecha_solucion = '$FechaAtendido'".$mas." WHERE id_reporte = $IdReporte";
	if(mysqli_query($conn, $sql)){
		$mensaje = '<script type="text/javascript">M.toast({html:"Reporte actualizado correctamente.", classes: "rounded"})</script>';
		echo '<script>
				location.href="../views/reportes.php";
			 </script>';
					echo '<script>recargar_rep()</script>';
	}else{
		$mensaje = '<script type="text/javascript">M.toast({html:"Ha ocurrido un error UPDATE.", classes: "rounded"})</script>';	
	}
